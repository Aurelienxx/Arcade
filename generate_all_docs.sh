...
find . -name "*.java" -not -path "./docs/*" -print0 | xargs -0 javadoc -d docs/javadoc 2>&1 | grep -E "(error|Error)" || echo "  ✅ Javadoc générée"
echo ""

# Python
echo " Génération documentation Python..."
pip install -q pdoc 2>/dev/null
PYTHON_COUNT=$(find . -name "*.py" -not -path "./docs/*" -not -path "./.git/*" | wc -l)
echo "  Fichiers Python trouvés: $PYTHON_COUNT"
if [ $PYTHON_COUNT -gt 0 ]; then
  mkdir -p docs/python
  pdoc -o docs/python scripts/ 2>/dev/null && echo "  ✅ Documentation Python générée" || echo "  ⚠️  Certains modules incomplets"
else
